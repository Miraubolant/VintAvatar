    ...
    try {
      ...
      if (data.success) {
        ...
        setTimeout(async () => {
          ...
          if (config.cropHead) {
            try {
              finalImageUrl = await cropHead(data.generated_image_url);
            } catch (cropError) {
              console.error('Erreur lors du crop automatique:', cropError);
    ...
    <section id="hero" className="relative min-h-screen bg-cream overflow-hidden">
      ...
      <div className="relative max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-16 lg:py-20">
        <div className="text-center space-y-12">
          ...
          {/* Call to action vers les tarifs */}
          <div className="max-w-3xl mx-auto mt-8">
            <div className="bg-white border-4 border-black p-4 shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] transform -rotate-1 relative">
              {/* Éléments décoratifs */}
              <div className="absolute -top-2 -right-2 w-6 h-6 bg-pink-pastel border-2 border-black transform rotate-45 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]"></div>
              <div className="absolute -bottom-2 -left-2 w-5 h-5 bg-mint border-2 border-black transform -rotate-12 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]"></div>

              <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 sm:w-12 sm:h-12 bg-vinted border-3 border-black flex items-center justify-center shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] transform rotate-3">
                    <Sparkles className="w-5 h-5 sm:w-6 sm:h-6 text-white" />
                  </div>
                  <div>
                    <p className="font-display font-bold text-sm sm:text-base text-black">
                      {t('cta.title', 'Prêt à transformer vos photos ?')}
                    </p>
                    <p className="font-body text-xs sm:text-sm text-gray-600 mt-0.5">
                      {t('cta.subtitle', 'Découvrez nos formules adaptées à vos besoins')}
                    </p>
                  </div>
                </div>
                ...
                <button
                  ...
                  className="w-full sm:w-auto px-6 py-3 bg-vinted text-white border-3 border-black font-display font-bold text-sm shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:translate-x-[-2px] hover:translate-y-[-2px] hover:shadow-[5px_5px_0px_0px_rgba(0,0,0,1)] transition-all duration-200 flex items-center justify-center gap-2"
                >
                  <CreditCard className="w-4 h-4" />
                  <span>{t('cta.button', 'VOIR LES TARIFS')}</span>
                </button>
              </div>
            </div>
          </div>
